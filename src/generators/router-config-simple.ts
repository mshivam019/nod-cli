...
export async function generateRouterConfigSimple(projectPath: string, ext: string) {
  const content = `/**
 ...
 * - wrapHandler: Wrap controller handlers for automatic response handling
 * - success: Create successful response
 * - fail: Create error response
 * - createError: Create error with status code
 */

export { METHODS, createConfiguredRouter, wrapHandler, success, fail, createError } from '../helpers/route-builder.js';
`;
